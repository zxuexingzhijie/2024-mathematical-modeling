...
received_signal = data['Received Signal Value'].values

# 步骤2：使用插值法进行数据重建
# 找出有效的信号部分，排除间歇性缺失部分
valid_indices = ~np.isnan(received_signal)  # 有效信号的索引
valid_time = time[valid_indices]  # 有效时间
valid_signal = received_signal[valid_indices]  # 有效信号

# 使用线性插值重建信号
interpolation_function = interp1d(valid_time, valid_signal, kind='linear', fill_value='extrapolate')
reconstructed_signal = interpolation_function(time)  # 使用插值法恢复信号

# 步骤3：使用FFT进行频谱分析
n = len(reconstructed_signal)
sampling_interval = time[1] - time[0]  # 假设时间值均匀分布
sampling_frequency = 1 / sampling_interval

# 进行FFT变换
fft_values = fft(reconstructed_signal)
...
# 使用滤波后的信号运行PSO
best_frequency, best_error = particle_swarm_optimization(num_particles, bounds, max_iter, reconstructed_signal, time)
